...
async function verifyHttpContract({ backendBaseUrl }) {
  ...
  assertSmoke(graphEntities.json?.total === 0, `Expected 0 graph entities, got ${graphEntities.json?.total}`)
  assertSmoke(graphRelationships.json?.total === 0, `Expected 0 graph relationships, got ${graphRelationships.json?.total}`)
...
async function runBrowserFlow({ backendBaseUrl }) {
  ...
  try {
    ...
    await page.getByTestId(GRAPH_SMOKE_TEST_IDS.emptyOverlay).waitFor({ timeout: 30000 })
    ...
    await page.screenshot({
      path: path.join(EVIDENCE_DIR, 'task-50-dashboard-stat-graph-empty-real.png'),
      fullPage: true,
    })
...
async function main() {
  ...
  await Promise.all([
    'task-50-dashboard-stat-graph-empty-real-summary.txt',
    'task-50-dashboard-stat-graph-empty-real-summary.json',
    'task-50-dashboard-stat-graph-empty-real-http.json',
    'task-50-dashboard-stat-graph-empty-real.png',
    'task-50-dashboard-stat-graph-empty-real-error.txt',
    'task-50-dashboard-stat-graph-empty-real-backend.log',
  ].map((fileName) => rm(path.join(EVIDENCE_DIR, fileName), { force: true })))

  await runBuildWeb(FRONTEND_ROOT)

  const workspaceRoot = await mkdtemp(path.join(os.tmpdir(), 'memory-graph-dashboard-stat-graph-empty-'))
  const backendPort = await findAvailablePort(Number(process.env.T50_BACKEND_PORT ?? '38500'))
  ...
  const backend = startBackendService({
    ...
    logFileName: 'task-50-dashboard-stat-graph-empty-real-backend.log',
    backendPort,
    extraEnv: {
  ...
  try {
    ...
    const summaryPayload = {
      ...
      command: 'npm --prefix frontend run qa:real-stack-smoke:dashboard:stats:graph:empty',
      status: 'passed',
      ...
      graph_entity_count: httpEvidence.graphEntities.json?.total,
      graph_relationship_count: httpEvidence.graphRelationships.json?.total,
      final_url: browserEvidence.finalUrl,
      screenshot: '.sisyphus/evidence/task-50-dashboard-stat-graph-empty-real.png',
      backend_log: '.sisyphus/evidence/task-50-dashboard-stat-graph-empty-real-backend.log',
    }
    ...
    await writeFile(path.join(EVIDENCE_DIR, 'task-50-dashboard-stat-graph-empty-real-summary.txt'), `${summaryText}\n`, 'utf8')
    await writeFile(path.join(EVIDENCE_DIR, 'task-50-dashboard-stat-graph-empty-real-summary.json'), `${JSON.stringify(summaryPayload, null, 2)}\n`, 'utf8')
    await writeFile(
      path.join(EVIDENCE_DIR, 'task-50-dashboard-stat-graph-empty-real-http.json'),
      `${JSON.stringify({
    ...
    await writeFile(
      path.join(EVIDENCE_DIR, 'task-50-dashboard-stat-graph-empty-real-error.txt'),
