...
async function verifyHttpContract({ backendBaseUrl, seedPayload }) {
  ...
  assertSmoke(
    query.json?.sources?.[0]?.community_id === targetCommunityId,
    `Expected query source community_id=${targetCommunityId}, got ${query.json?.sources?.[0]?.community_id}`
  )

  const communityDetail = await requestJson(`${backendBaseUrl}/api/v1/communities/${targetCommunityId}`)
...
async function runBrowserFlow({ backendBaseUrl, targetCommunityId }) {
  ...
  try {
    ...
    await page.waitForSelector(`text=Failed to get community entities: QA forced failure for community entities: ${targetCommunityId}`, { timeout: 30000 })
    await page.waitForSelector(`text=Failed to get community relationships: QA forced failure for community relationships: ${targetCommunityId}`, { timeout: 30000 })

    await page.screenshot({
      path: path.join(EVIDENCE_DIR, 'task-60-search-source-community-data-failure-real.png'),
      fullPage: true,
    })
...
async function main() {
  ...
  await Promise.all([
    'task-60-search-source-community-data-failure-real-summary.txt',
    'task-60-search-source-community-data-failure-real-summary.json',
    'task-60-search-source-community-data-failure-real-http.json',
    'task-60-search-source-community-data-failure-real-seed.json',
    'task-60-search-source-community-data-failure-real.png',
    'task-60-search-source-community-data-failure-real-error.txt',
    'task-60-search-source-community-data-failure-real-backend.log',
  ].map((fileName) => rm(path.join(EVIDENCE_DIR, fileName), { force: true })))

  await runBuildWeb(FRONTEND_ROOT)

  const workspaceRoot = await mkdtemp(path.join(os.tmpdir(), 'memory-graph-search-source-community-data-failure-'))
  const backendPort = await findAvailablePort(Number(process.env.T60_BACKEND_PORT ?? '38600'))
  const ollamaPort = await findAvailablePort(Number(process.env.T60_OLLAMA_PORT ?? '38601'))
  ...
  const seedOutputPath = path.join(EVIDENCE_DIR, 'task-60-search-source-community-data-failure-real-seed.json')
  ...
  const backend = startBackendService({
    ...
    logFileName: 'task-60-search-source-community-data-failure-real-backend.log',
    backendPort,
    extraEnv: {
  ...
  try {
    ...
    const summaryPayload = {
      ...
      command: 'npm --prefix frontend run qa:real-stack-smoke:search:source-community:data-failure',
      status: 'passed',
      ...
      link_surface: 'source',
      community_title: httpEvidence.communityDetail.json?.title,
      ...
      screenshot: '.sisyphus/evidence/task-60-search-source-community-data-failure-real.png',
      backend_log: '.sisyphus/evidence/task-60-search-source-community-data-failure-real-backend.log',
      seed_fixture: '.sisyphus/evidence/task-60-search-source-community-data-failure-real-seed.json',
    }
    ...
    const summaryText = [
      ...
      `community_title=${summaryPayload.community_title}`,
    ...
    await writeFile(path.join(EVIDENCE_DIR, 'task-60-search-source-community-data-failure-real-summary.txt'), `${summaryText}\n`, 'utf8')
    await writeFile(path.join(EVIDENCE_DIR, 'task-60-search-source-community-data-failure-real-summary.json'), `${JSON.stringify(summaryPayload, null, 2)}\n`, 'utf8')
    await writeFile(
      path.join(EVIDENCE_DIR, 'task-60-search-source-community-data-failure-real-http.json'),
      `${JSON.stringify({
    ...
    await writeFile(
      path.join(EVIDENCE_DIR, 'task-60-search-source-community-data-failure-real-error.txt'),
