...
const MISSING_COMMUNITY_ID = 'comm-missing-search-stale-link-real-smoke'
const INVOKED_COMMAND = process.env.npm_lifecycle_event
  ? `npm --prefix frontend run ${process.env.npm_lifecycle_event}`
  : 'npm --prefix frontend run qa:real-stack-smoke:search:community:stale-link'
...
async function verifyHttpContract({ backendBaseUrl, seedPayload }) {
  ...
  assertSmoke(query.json?.sources?.[0]?.community_id === seedPayload.community_id, `Expected source community_id=${seedPayload.community_id}, got ${query.json?.sources?.[0]?.community_id}`)
  assertSmoke(query.json?.communities?.[0]?.community_id === seedPayload.community_id, `Expected community hit ${seedPayload.community_id}, got ${query.json?.communities?.[0]?.community_id}`)
...
async function runBrowserFlow({ backendBaseUrl, seedPayload }) {
  ...
  try {
    ...
    assertSmoke((await page.getByTestId(COMMUNITIES_SMOKE_TEST_IDS.detailPanel).count()) === 0, 'Stale community hash should clear existing detail panel when entered from Search')

    await page.screenshot({
      path: path.join(EVIDENCE_DIR, 'task-39-search-community-stale-link-real.png'),
      fullPage: true,
    })
...
async function main() {
  ...
  await Promise.all([
    'task-39-search-community-stale-link-real-summary.txt',
    'task-39-search-community-stale-link-real-summary.json',
    'task-39-search-community-stale-link-real-http.json',
    'task-39-search-community-stale-link-real-seed.json',
    'task-39-search-community-stale-link-real.png',
    'task-39-search-community-stale-link-real-error.txt',
    'task-39-search-community-stale-link-real-backend.log',
  ].map((fileName) => rm(path.join(EVIDENCE_DIR, fileName), { force: true })))

  await runBuildWeb(FRONTEND_ROOT)

  const workspaceRoot = await mkdtemp(path.join(os.tmpdir(), 'memory-graph-search-community-stale-link-'))
  const backendPort = await findAvailablePort(Number(process.env.T39_BACKEND_PORT ?? '38390'))
  const ollamaPort = await findAvailablePort(Number(process.env.T39_OLLAMA_PORT ?? '38391'))
  ...
  const seedOutputPath = path.join(EVIDENCE_DIR, 'task-39-search-community-stale-link-real-seed.json')
  ...
  const backend = startBackendService({
    ...
    logFileName: 'task-39-search-community-stale-link-real-backend.log',
    backendPort,
    extraEnv: {
  ...
  try {
    ...
    const summaryPayload = {
      ...
      screenshot: '.sisyphus/evidence/task-39-search-community-stale-link-real.png',
      backend_log: '.sisyphus/evidence/task-39-search-community-stale-link-real-backend.log',
      seed_fixture: '.sisyphus/evidence/task-39-search-community-stale-link-real-seed.json',
    }
    ...
    await writeFile(path.join(EVIDENCE_DIR, 'task-39-search-community-stale-link-real-summary.txt'), `${summaryText}\n`, 'utf8')
    await writeFile(path.join(EVIDENCE_DIR, 'task-39-search-community-stale-link-real-summary.json'), `${JSON.stringify(summaryPayload, null, 2)}\n`, 'utf8')
    await writeFile(
      path.join(EVIDENCE_DIR, 'task-39-search-community-stale-link-real-http.json'),
      `${JSON.stringify({
    ...
    await writeFile(
      path.join(EVIDENCE_DIR, 'task-39-search-community-stale-link-real-error.txt'),
