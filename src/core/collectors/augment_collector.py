...
class AugmentConfig:
    ...
    def _find_data_dir(self) -> str:
        ...
        if os.name == "nt":  # Windows
            ...
            return os.path.join(base, "Augment")
        else:
            return os.path.expanduser("~/.augment")


class AugmentCollector(BaseCollector):
    """
    Augment 采集器
    企业级代码增强 AI 助手
    支持自定义规则和工作流配置
    ...
    def _find_data_dirs(self) -> List[str]:
        ...
        for project_dir in self._config.project_dirs:
            if os.path.isdir(project_dir):
                # 检查 .augment 目录
                augment_dir = os.path.join(project_dir, ".augment")
                if os.path.isdir(augment_dir):
                    data_dirs.append(augment_dir)
    ...
    async def _process_file(self, file_path: str):
        ...
        try:
            ...
            if messages:
                ...
                logger.info(f"Imported Augment conversation: {file_path}")
...
# 全局实例
_augment_collector_instance: Optional[AugmentCollector] = None


def get_augment_collector(config: Optional[Dict[str, Any]] = None) -> AugmentCollector:
    """获取 Augment 采集器单例"""
    global _augment_collector_instance
    if _augment_collector_instance is None:
        _augment_collector_instance = AugmentCollector(config)
    return _augment_collector_instance
