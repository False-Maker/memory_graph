...
const INVOKED_COMMAND = process.env.npm_lifecycle_event
  ...
  : 'npm --prefix frontend run qa:real-stack-smoke:search:community:summary:facet-failure'
...
async function runBrowserFlow({ backendBaseUrl }) {
  ...
  try {
    ...
    await page.screenshot({
      path: path.join(EVIDENCE_DIR, 'task-44-search-community-summary-facet-failure-real.png'),
      fullPage: true,
    })
...
async function main() {
  ...
  await Promise.all([
    'task-44-search-community-summary-facet-failure-real-summary.txt',
    'task-44-search-community-summary-facet-failure-real-summary.json',
    'task-44-search-community-summary-facet-failure-real-http.json',
    'task-44-search-community-summary-facet-failure-real-seed.json',
    'task-44-search-community-summary-facet-failure-real.png',
    'task-44-search-community-summary-facet-failure-real-error.txt',
    'task-44-search-community-summary-facet-failure-real-backend.log',
  ].map((fileName) => rm(path.join(EVIDENCE_DIR, fileName), { force: true })))

  await runBuildWeb(FRONTEND_ROOT)

  const workspaceRoot = await mkdtemp(path.join(os.tmpdir(), 'memory-graph-search-community-summary-facet-failure-'))
  const backendPort = await findAvailablePort(Number(process.env.T44_BACKEND_PORT ?? '38440'))
  const ollamaPort = await findAvailablePort(Number(process.env.T44_OLLAMA_PORT ?? '38441'))
  ...
  const seedOutputPath = path.join(EVIDENCE_DIR, 'task-44-search-community-summary-facet-failure-real-seed.json')
  ...
  const backend = startBackendService({
    ...
    logFileName: 'task-44-search-community-summary-facet-failure-real-backend.log',
    backendPort,
    extraEnv: {
  ...
  try {
    ...
    assertSmoke(afterSummary.json?.summary === browserEvidence.summarizePayload.summary, 'Expected refreshed summary to persist after Search -> Community summarize')
    ...
    const summaryPayload = {
      ...
      screenshot: '.sisyphus/evidence/task-44-search-community-summary-facet-failure-real.png',
      backend_log: '.sisyphus/evidence/task-44-search-community-summary-facet-failure-real-backend.log',
      seed_fixture: '.sisyphus/evidence/task-44-search-community-summary-facet-failure-real-seed.json',
    }
    ...
    await writeFile(path.join(EVIDENCE_DIR, 'task-44-search-community-summary-facet-failure-real-summary.txt'), `${summaryText}\n`, 'utf8')
    await writeFile(path.join(EVIDENCE_DIR, 'task-44-search-community-summary-facet-failure-real-summary.json'), `${JSON.stringify(summaryPayload, null, 2)}\n`, 'utf8')
    await writeFile(
      path.join(EVIDENCE_DIR, 'task-44-search-community-summary-facet-failure-real-http.json'),
      `${JSON.stringify({
    ...
    await writeFile(
      path.join(EVIDENCE_DIR, 'task-44-search-community-summary-facet-failure-real-error.txt'),
